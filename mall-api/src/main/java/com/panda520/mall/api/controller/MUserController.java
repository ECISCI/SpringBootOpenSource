...
import com.panda520.mall.api.entity.login.LoginReq;
import com.panda520.mall.api.entity.login.LoginRes;
import com.panda520.mall.api.entity.register.RegisterReq;
import com.panda520.mall.api.util.Constant;
import com.panda520.mall.api.util.JWTUtil;
import com.panda520.mall.api.util.UUidUtils;
...
public class MUserController extends BaseController {
    ...
    public ResponseResult mobileRegister(@RequestBody RegisterReq requestData) {
        ...
        // 如果用不为空,证明该用户已存在
        if (user != null) {
            return ResponseResult.error(Constant.REGISTER_EXIST);
        }
        ...
        if (insertCode == 0) { // 插入数据库失败
            return ResponseResult.error(Constant.REGISTER_FAILED);
        }
        return ResponseResult.success(Constant.REGISTER_SUCCESS);
    }
}
